...
async def on_device_connected(device_id: str):
    ...
    _LOG.debug("Broadlink connected: %s", device_id)
    state = media_player.States.UNKNOWN
    if device_id not in _configured_devices:
        _LOG.warning("Broadlink %s is not configured", device_id)
        return
...
async def on_device_disconnected(device_id: str):
    ...
    _LOG.debug("Broadlink disconnected: %s", device_id)
...
async def on_device_connection_error(device_id: str, message):
    """Set entities of Broadlink to state UNAVAILABLE if device connection error occurred."""
    _LOG.error(message)
...
def _device_state_to_media_player_state(
    device_state: rm.PowerState,
) -> media_player.States:
    match device_state:
        case rm.PowerState.ON:
            state = media_player.States.ON
        case rm.PowerState.OFF:
            state = media_player.States.OFF
        case rm.PowerState.STANDBY:
...
def _add_configured_device(
    device_config: BroadlinkDevice, connect: bool = False
) -> None:
    ...
    if device_config.identifier in _configured_devices:
        ...
        device = rm.Broadlink(device_config, loop=_LOOP)
        device.events.on(rm.EVENTS.CONNECTED, on_device_connected)
        device.events.on(rm.EVENTS.DISCONNECTED, on_device_disconnected)
        device.events.on(rm.EVENTS.ERROR, on_device_connection_error)
        device.events.on(rm.EVENTS.UPDATE, on_device_update)
...
def _register_available_entities(
    device_config: BroadlinkDevice, device: rm.Broadlink
) -> bool:
    """
...
def on_device_removed(device: BroadlinkDevice | None) -> None:
