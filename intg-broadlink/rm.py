...
class Broadlink:
    ...
    def convert_to_pronto(self, code: str, code_type: str = "auto") -> str:
        """
        Convert custom IR codes to PRONTO format.

        Args:
            code: IR code string (custom format)
            code_type: Type of code ("custom" or "auto" for auto-detection)
        ...
        Raises:
            ValueError: If code format is invalid or conversion functions not available
        """
        if custom_to_pronto is None:
            raise ValueError("Custom IR converter function not available")
        ...
        if code_type == "auto" or code_type is None:
            if ";" in code and code.count(";") == 3:
                ...
                raise ValueError("Only custom semicolon-separated format is supported for PRONTO conversion")

        try:
            if code_type.lower() == "custom":
                return custom_to_pronto(code)
            else:
                raise ValueError(f"Unsupported code type for PRONTO conversion: {code_type}")

        except Exception as e:
            _LOG.error("[%s] Error converting IR code to PRONTO: %s", self.log_id, e)
            raise ValueError(f"Failed to convert IR code to PRONTO: {e}") from e
